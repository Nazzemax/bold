...
const siteCondition: string[] = [
  "Новый сайт",
  "Есть сайт, но требуется доработка",
];
...
export const FormSEOOptimization: React.FC = () => {
  ...
  const [selectedOptionsSiteCondition, setSelectedOptionsSiteCondition] =
    useState<string[]>([]);
  const [isOpenSiteCondition, setIsOpenSiteCondition] =
                  ...
                  <div className="absolute w-full bg-white text-[#2A2D35] mt-1 rounded-xl shadow-lg max-h-40 overflow-y-auto z-10">
                    {siteCondition.map((option, index) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={selectedOptionsSiteCondition.includes(
                            option
                          )}
