...
import "../../../components/layout/cases/CasesIntro.scss"
...
const BreadcrumbCases: React.FC = () => {
  ...
  return (
    <nav className="flex text-[#838895]">
      <Link href="/" className="hover:underline text-[#838895]">
        Главная
      </Link>
      {breadcrumbItems.map((item, index) => (
        <span key={item.href} className="flex items-center">
          ...
          {index === breadcrumbItems.length - 1 ? (
            <span className="text-[#838895]">{item.name}</span>
          ) : (
